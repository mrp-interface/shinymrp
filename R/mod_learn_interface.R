  ...
  tagList(
    fluidRow(
      column(width = 6, offset = 3,
        ...
        tags$p("The prior specification can be modified by users. Users can choose the structured prior distribution for the two-way interaction terms, developed by ", tags$a("Si et al. (2022)", href = "https://arxiv.org/abs/1707.08220", target = "_blank"), ". For example, the structured prior distribution for the two-way interaction of race and age is specified as below:"),
        ...
        tags$p("The model fitting process involves generating MCMC chains which are usually done in parallel to reduce runtime. The interface automatically use one core for each chain so the user must choose the number of chains carefully based on the available computing resouces. Learn more about sampling ",
