      ...
      bslib::card(
        ...
        bslib::card_body(
          bslib::value_box(
            ...
            p(withMathJax("1. Fit a multilevel model for the response with the adjustment variables based on the input data")),
            p(withMathJax("2. Poststratify using the population distribution of the adjustment variables, yielding estimates in the target population and subgroups"))
          ),
          bslib::accordion(
            id = "mrp_sections",
            open = TRUE,
            ...
            bslib::accordion_panel(
              ...
              tags$div(class = "learn_math",
                ...
                withMathJax("where:"),
                tags$ul(
                  tags$li(withMathJax("\\(\\alpha_{\\rm a}^{\\rm age}\\) is the effect of subject \\(i\\)'s age on the log-odds function of the probability of having a positive response.")),
                  tags$li(withMathJax("\\(\\alpha_{\\rm r}^{\\rm eth}\\) is the effect of subject \\(i\\)'s race/ethnicity on the probability of having a positive response.")),
                  tags$li(withMathJax("\\(\\alpha_{\\rm e}^{\\rm educ}\\) is the effect of subject \\(i\\)'s education on the probability of having a positive response.")),
                  tags$li(withMathJax("\\(\\alpha_{\\rm s}^{\\rm state}\\): The effect of subject \\(i\\)'s state on the probability of having a positive response. As we have state-level predictors, we need to build another model in which \\(\\alpha_{\\rm s}^{\\rm state}\\) is the outcome of a linear regression with state-level predictors. For state \\(s\\),"))
                ),
            ...
            bslib::accordion_panel(
              "MRP for spatio-temporal data with measurement error",
              tags$div(class = "learn_math",
                ...
                withMathJax("We will start by fitting a Binomial model for \\(y^*_k\\), \\(y^*_k \\sim \\textrm{Binomial}(n_k, p_k)\\) with a logit function for \\(\\pi_k\\) with covariates including sex, age, race, zip codes, and time in weeks to allow time variation of positive response rate over time in the multilevel model."),
