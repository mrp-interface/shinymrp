...
The `MRPModel` class is designed to represent individual models and provides methods for
interacting with the CmdStanR objects under the hood, from running MCMC to extracting
posterior samples and processing them into a format suitable for visualization.
...
it uses a 95% credible interval for uncertainty quantification but users can specify other intervals.
...
```{r, estimate_map}
workflow$estimate_map(model1, geo = "county", interval = 0.95)
```
