...
The `MRPModel` class is designed to represent individual models within the workflow and contains methods for model fitting, diagnostics, and visualization.

### Model specification

The `$create_model()` method takes in a nested list that specifies the variables to include in the model,
whether they are included as fixed or varying effects, selected interactions and the prior distributions assigned to all the effects.
This is used to generate Stan code for compilation using the `CmdStanR` package.
...
workflow$load_model("model1.qs")
```


## Visualize estimates

The workflow object provides visualization methods to present the results of the fitted models,
including overall estimates and estimates for demographic groups or geographic areas. 

```{r, estimate_plot, fig.height=10}
workflow$estimate_plot(model1, group = "sex")
```

For geography-based results, users have the option to visualize the values on an interactive choropleth map.
